...
export function carTextureKey(frameIndex: number, numFrames = CAR.NUM_ANGLE_FRAMES): string {
  const step = 360 / numFrames;
  // The asset filenames were generated by Python's round() (banker's
  // rounding: ties go to the even integer — round(22.5) = 22, round(23.5) = 24).
  // JS's Math.round rounds half toward +Infinity, which mismatches on the
  // .5 ties (22.5 → 23 in JS, 22 in Python). Use a banker's-round helper
  // so the keys line up with the actual files (car_000, car_011, car_022,
  // car_034, …, car_292, car_304, …, car_349).
  const angle = bankersRound(frameIndex * step) % 360;
  // Files are zero-padded to 3 digits: car_000.png, car_011.png, ...
  return `car_${String(angle).padStart(3, '0')}`;
}

/** Round half-to-even (Python-style). Used to match the spritesheet naming. */
